...
from vehicles._vehicle import Ackermann
...
vehicle = Ackermann(cur_x=x[0,0], cur_y=x[1,0], cur_rad_1=x[2,0], cur_rad_2=x[3,0], ell_W=ELL_W, ell_T=ELL_T, tick_speed=T)

# Run the simulation
for k in range(1, N):
    vehicle = vehicle.control(u[:,k-1])

    x[:, k] = vehicle.cur #rk_four(vehicle.f, x[:, k - 1], u[:, k - 1], T)
    phi_L[k] = np.arctan(
...
from OLD.mobotpy.models import Ackermann
...
# Save the plot
plt.savefig("./ackermann_kinematic_fig1.pdf")
...
# Save the plot
plt.savefig("./ackermann_kinematic_fig2.pdf")
