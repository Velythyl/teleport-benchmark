...
import vehicles._vehicle
from vehicles.OLD.mobotpy.models import Tricycle
...
vehicle = vehicles._vehicle.Trike(cur_x=x[0,0], cur_y=x[1,0], cur_rad_1=x[2,0], cur_rad_2=x[3,0], ell_W=ELL_W, ell_T=ELL_T, tick_speed=T)

# Run the simulation
for k in range(1, N):
    vehicle = vehicle.control(u[:, k-1])
    x[:, k] = vehicle.cur #rk_four(tricycle_f, x[:, k - 1], u[:, k - 1], T)
    u[0, k] = 5.0
...
# Save the plot
plt.savefig("./tricycle_kinematic_fig1.pdf")
...
# Save the plot
plt.savefig("./tricycle_kinematic_fig2.pdf")
...
ani = vehicle.animate(x, T, True, "./tricycle_kinematic.gif")
